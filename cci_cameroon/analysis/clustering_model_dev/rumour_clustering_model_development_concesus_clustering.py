...
# * Create adjacency matrix using the similarity scores. Entry i,j =1 if node j is among top 10 similar nodes to j, and 0 otherwise. Generate a connectivity graph using the adjacency matrix.
# * Using a community detection algorithm, we generate subgroups/clusters from the graph
# * Generate an ensemble of clusters from repeated runs of the community detection algorithm and compute pairwise adjusted mutual inforation score to determine cluster robustness.
#
...
indsamp = faiss.index_factory(
    sample_data_embeddings.shape[1], "Flat", faiss.METRIC_INNER_PRODUCT
)
